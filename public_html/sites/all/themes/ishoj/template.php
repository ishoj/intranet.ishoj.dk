...
function ishoj_preprocess_page(&$variables) {
  ...
  if($variables['logged_in']) {
    // Indlæs editor.css
    drupal_add_css (path_to_theme() . '/dist/css/editor.css', array('type' => 'file'));
    // Indlæs editor.js
    drupal_add_js(drupal_get_path('theme', 'ishoj') . '/dist/js/editor.min.js');
  }
...
//  } 
  ...
  // Hvis det er aktivitetssiden
  if(strpos($urlPath, 'aktiviteter') !== false) { 
     $urlPath = 'aktivitetsside';
  } 
...
function ishoj_preprocess_node(&$variables) {
  ...
  if(($node->type == 'thomas_tester') or 
    ...
    drupal_add_js(path_to_theme() . '/dist/js/google-maps.min.js', array(
      'group' => JS_THEME,
      'preprocess' => TRUE,
      'scope' => 'footer',
      'weight' => '999',
